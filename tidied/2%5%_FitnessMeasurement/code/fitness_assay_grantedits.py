...
### GRK 28April2022 - Note previous nansum wrapper wasn't behaving as expected - with numpy update was return 0 if all were np.nan
### Now, if all are null, returns null.
...
def meanVarAndNeutrals(neutralIndices,replicateReads,zCutoff,cycleTimes,use_all_neutral,firstPass = False):
    ...
    for initIdx,initTime in enumerate(cycleTimes[0:-1]):
        ...
        if use_all_neutral:
            ...
            print(f'Number of Neutrals used:{len(replicateReads[neutralIndices, initIdx])}; FirstPass:{firstPass}; initReads:{initReads}; finalReads:{finalReads}')
