...
SetDirectory["/Users/grantkinsler/Documents/Stanford/Research/EvolvingFront/EvolutionTracking/Mathematica_DFE_fromJamie/data"];
condition ="Evo2D_Ira1_Non_1"
AllTrajectories=Import[StringForm["``_counts.txt",condition], "Table"];
...
(*TimePoints={0,8,32,56,80,104,128,152,176,192};*)
TimePoints={0,8,32,56,80,112,136,168};
...
(*CombineTimePoints[Trajectory_]:=Module[{t0,t8,t32,t56,t80,t104,t128,t152,t176,t192},
	t0={{    0,If[   Trajectory[[1]]==0,1, Trajectory[[1]]  ]     }};
	t8={{  8,Trajectory[[2]]    }};
	t32={{  32,Trajectory[[3]]     }};
	t56={{  56,Trajectory[[4]]     }};
	t80={{  80,Trajectory[[5]]     }};
	t104={{  104,Trajectory[[6]]     }};
	t128={{  128,Trajectory[[7]]     }};
	t152={{  152,Trajectory[[8]]     }};
	t176={{  176,Trajectory[[9]]     }};
	t192={{  192,Trajectory[[10]]   }};


	Join[t0,t8,t32,t56,t80,t104,t128,t152,t176,t192]
]; (* before join, join[ {{0,xx}}, {{16,xx}}, ....]; after join, {0,xx},{16,xx}, ...; thus each row is a [length(time_points) * 2] matrix *)*)

CombineTimePoints[Trajectory_]:=Module[{t0,t8,t32,t56,t80,t112,t136,t168},
	t0={{    0,If[   Trajectory[[1]]==0,1, Trajectory[[1]]  ]     }};
	t8={{  8,Trajectory[[2]]    }};
	t32={{  32,Trajectory[[3]]     }};
	t56={{  56,Trajectory[[4]]     }};
	t80={{  80,Trajectory[[5]]     }};
	t112={{  104,Trajectory[[6]]     }};
	t136={{  128,Trajectory[[7]]     }};
	t168={{  152,Trajectory[[8]]     }};


	Join[t0,t8,t32,t56,t80,t112,t136,t168]
]; (* before join, join[ {{0,xx}}, {{16,xx}}, ....]; after join, {0,xx},{16,xx}, ...; thus each row is a [length(time_points) * 2] matrix *)
...
Export[StringForm["ReadTrajectories_``.csv",condition],ReadTrajectories2M3,"CSV"];
...
Export[StringForm["ReadDepths_``.csv",condition],ReadDepths2M3,"CSV"];


(*Convert read trajectories to approximate number trajectories for plotting purposes. Export to file.*)
ReadTrajectories2M3=ToExpression[Import[StringForm["ReadTrajectories_``.csv",condition]]];
ReadDepths2M3=ToExpression[Import[StringForm["ReadDepths_``.csv",condition]]];
...
Export["CellNumberTrajectories_Evo1D_IRAs_1.csv", CellNumberTrajectories2M3, "CSV"];
Export["ExtinctLineages_Evo1D_IRAs_1.csv", ExtinctLineages, "CSV"];
...
ReadTrajectories2M3=ToExpression[Import["ReadTrajectories_Evo1D_IRAs_1.csv"]];
ReadDepths2M3=ToExpression[Import["ReadDepths_Evo1D_IRAs_1.csv"]];
...
Export["InferredxBarAndKappa_Evo1D_IRAs_1.csv", AllxBarKappaFits, "CSV"];
Export["PlotsOfInferredxBarAndKappa_Evo1D_IRAs_1.pdf", TableForm[GridOfFittedPlots, TableHeadings -> {Table[TimePoints[[i]], {i, 1, Length[TimePoints]}],Table[r, {r, LowerReadLimit,UpperReadLimit}]}], "PDF"];
(* Figure 17 & 19 *)
...
ReadDepths2M3=ToExpression[Import["ReadDepths_Evo1D_IRAs_1.csv"]];
ReadTrajectories2M3=ToExpression[Import["ReadTrajectories_Evo1D_IRAs_1csv"]]; (*all lines*)
CellNumberTrajectories2M3=ToExpression[Import["CellNumberTrajectories_Evo1D_IRAs_1.csv"]]; (*all lines*)
ClearMemory
...
xBarKappa2M3=ToExpression[Import["InferredxBarAndKappa_Evo1D_IRAs_1.csv"]];
...
\*SubscriptBox[\(\[PartialD]\), \(\[Tau]\)]\ ProbSAndTauGivenData2M3[InferredS2M3, \ \[Tau]]\)\)/. \[Tau] -> InferredTau2M3;
		...
		Export[StringJoin["Evo1D_IRAs_1_",ToString[TableOfData[[1]][[1]]],"-",ToString[TableOfData[[50000]][[1]]],".CSV"],TableOfData,"CSV"]; (* all lines *)
		TableOfData={};
		];
...
(*all lines *)
Export[StringJoin["Evo1D_IRAs_1_",ToString[TableOfData[[1]][[1]]],"-",ToString[TableOfData[[Length[TableOfData]]][[1]]],".CSV"],TableOfData,"CSV"];
Exit[]
